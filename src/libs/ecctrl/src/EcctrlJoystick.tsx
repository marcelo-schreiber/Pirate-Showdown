...
const JoystickComponents = (props: EcctrlJoystickProps) => {
    ...
    const onTouchMove = useCallback((e: TouchEvent) => {
        ...
        joystickDis.current = Math.min(Math.sqrt(Math.pow(touch1MovementX, 2) + Math.pow(touch1MovementY, 2)), joystickMaxDis.current)
        joystickAng.current = touch1MovementVec2.angle()
        joystickMovementVec2.set(joystickDis.current * Math.cos(joystickAng.current), joystickDis.current * Math.sin(joystickAng.current))
        const runState = joystickDis.current > joystickMaxDis.current * (props.joystickRunSensitivity ?? 0.9)
    ...
    }, [api, joystickMovementVec2, props.joystickRunSensitivity, setJoystick, touch1MovementVec2, windowSize])
