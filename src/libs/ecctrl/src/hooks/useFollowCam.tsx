...
export const useFollowCam = function ({
  ...
  const onDocumentMouseMove = useCallback((e: MouseEvent) => {
    ...
    return false;
  }, [camLowLimit, camMoveSpeed, camUpLimit, followCam.position, followCam.rotation, pivot.rotation]);

  // Mouse scroll event
  const onDocumentMouseWheel = useCallback((e: Event) => {
    ...
    return false;
  }, [camMaxDis, camMinDis, camZoomSpeed, followCam.position, followCam.rotation.x]);
  ...
  }, [camLowLimit, camMaxDis, camMinDis, camMoveSpeed, camUpLimit, camZoomSpeed, followCam.position, followCam.rotation, pivot.rotation]);
  ...
  const cameraCollisionDetect = (delta: number) => {
    ...
    if (intersects.current.length && intersects.current[0].distance <= -originZDis.current) {
      smallestDistance.current = Math.min(-intersects.current[0].distance * camCollisionOffset, camMinDis)
    } else {
  ...
  useEffect(() => {
    ...
    if (camListenerTarget === "domElement") {
      gl.domElement.addEventListener("mousedown", () => { isMouseDown.current = true });
      gl.domElement.addEventListener("mouseup", () => { isMouseDown.current = false });
    ...
    return () => {
      if (camListenerTarget === "domElement") {
        gl.domElement.removeEventListener("mousedown", () => { isMouseDown.current = true });
        gl.domElement.removeEventListener("mouseup", () => { isMouseDown.current = false });
        ...
        document.removeEventListener("mousedown", () => { isMouseDown.current = true });
        document.removeEventListener("mouseup", () => { isMouseDown.current = false });
  ...
  }, [camInitDir.x, camInitDir.y, camListenerTarget, customTraverseAdd, followCam, gl.domElement, onDocumentMouseMove, onDocumentMouseWheel, onTouchEnd, onTouchMove, pivot, scene])
  ...
  useEffect(() => {
    const onObjectAdded = (e: { child: THREE.Object3D }) => customTraverseAdd(e.child)
    const onObjectRemoved = (e: { child: THREE.Object3D }) => customTraverseRemove(e.child)
