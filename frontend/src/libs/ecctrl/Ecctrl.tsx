...
const Ecctrl: ForwardRefRenderFunction<CustomEcctrlRigidBody, EcctrlProps> = (
  ...
  if (debug) {
    ...
    turnVelMultiplier =
      characterControlsDebug.turnVelMultiplier ?? turnVelMultiplier;
    ...
    jumpForceToGroundMult =
      characterControlsDebug.jumpForceToGroundMult ?? jumpForceToGroundMult;
    ...
    airDragMultiplier =
      characterControlsDebug.airDragMultiplier ?? airDragMultiplier;
    ...
    moveImpulsePointY =
      characterControlsDebug.moveImpulsePointY ?? moveImpulsePointY;
  ...
  const rayHit = useRef<RayColliderHit | null>(null);
  ...
  const slopeRayHit = useRef<RayColliderHit | null>(null);
  ...
  useEffect(() => {
    if (!animated) return;
    if (!isInsideKeyboardControls || !subscribeKeys) return;
  ...
  useFrame((state, delta) => {
     ...
     */
    if (followLight && dirLight.current && characterModelRef.current) {
    ...
    /**Test shape ray */
    // rayHit.current = world.castShape(
    //   currentPos,
    //   { w: 0, x: 0, y: 0, z: 0 },
    //   {x:0,y:-1,z:0},
    //   shape,
    ...
    if (
      rayHit.current &&
      rayHit.current.timeOfImpact < floatingDis + rayHitForgiveness
    ) {
      if (slopeRayHit.current && actualSlopeAngle < slopeMaxAngle) {
        canJump = true;
      }
    } else {
      canJump = false;
     ...
     */
    if (rayHit.current && canJump) {
      if (rayHit.current.collider.parent()) {
        // Getting the standing force apply point
        standingForcePoint.set(
          rayOrigin.x,
          rayOrigin.y - rayHit.current.timeOfImpact,
          rayOrigin.z,
        );
        const rayHitObjectBodyType = rayHit.current.collider
          .parent()
          ?.bodyType();
        const rayHitObjectBodyMass = rayHit.current.collider.parent()?.mass();
        ...
        if (rayHitObjectBodyType === 0 || rayHitObjectBodyType === 2) {
          ...
          distanceFromCharacterToObject
            .copy(currentPos)
            .sub(
              rayHit.current.collider.parent()?.translation() as THREE.Vector3,
            );
          // Moving object linear velocity
          const movingObjectLinvel = rayHit.current.collider
            .parent()
            ?.linvel() as THREE.Vector3;
          // Moving object angular velocity
          const movingObjectAngvel = rayHit.current.collider
            .parent()
    ...
    slopeRayHit.current = world.castRay(
      slopeRayCast,
      slopeRayLength,
      false,
      QueryFilterFlags.EXCLUDE_SENSORS,
    ...
    if (slopeRayHit.current) {
      actualSlopeNormal =
        slopeRayHit.current.collider.castRayAndGetNormal(
          slopeRayCast,
          slopeRayLength,
          false,
        )?.normal ?? null;
    ...
    if (
      slopeRayHit.current &&
      rayHit.current &&
      slopeRayHit.current.timeOfImpact < floatingDis + 0.5
    ) {
      if (canJump) {
     ...
     */
    if (rayHit.current != null) {
      if (canJump && rayHit.current.collider.parent()) {
        floatingForce =
          springK * (floatingDis - rayHit.current.timeOfImpact) -
    ...
    if (animated) {
      if (
        ...
        if (jumpIdleAnimation) jumpIdleAnimation();
      }
      // On high sky, play falling animation
      if (rayHit.current == null && isFalling) {
        if (fallAnimation) fallAnimation();
      }
    }
  });
